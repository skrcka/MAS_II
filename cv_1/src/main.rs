...
fn main() {
    ...
    let sparse_matrix_copy2_arc = std::sync::Arc::new(std::sync::Mutex::new(&sparse_matrix_copy2));
    let start = std::time::Instant::now();
    let sum: usize = sparse_matrix_copy
        .into_par_iter()
        .map(|(_, v)| {
            sparse_matrix_copy2_arc
                .lock()
                .unwrap()
                .iter()
    ...
    let start = std::time::Instant::now();
    let mut clustering_distribution: HashMap<usize, usize> = HashMap::new();
    let clustering_distribution_arc =
        std::sync::Arc::new(std::sync::Mutex::new(&mut clustering_distribution));
    sparse_matrix_copy.into_par_iter().for_each(|(_, v)| {
        let count = sparse_matrix_copy2_arc
            .lock()
            .unwrap()
            .iter()
            .filter(|(k, _)| v.contains_key(k))
            .count();
        clustering_distribution_arc
            .lock()
            .unwrap()
            .entry(count)
            .and_modify(|e| *e += 1)
...
/*
Average degree par: 7.9715676988139865 in 77
Average degree: 7.9715676988139865 in 7
Max degree: 28576 in 10
Degree distribution in 203
Clustering effect: 8.155038221913898 in 83469
Clustering distribution in 84089
Average common neighbors: 8.155038221913898 in 82540
Maximum common neighbors: 152258 in 82767
*/
